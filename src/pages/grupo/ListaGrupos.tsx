...
export const ListaGrupos: React.FC = () => {
    ...
    useEffect(() => {
        ...
        debounce(() => {
            GrupoService.getAll(parseFloat(localStorage.getItem(Environment.APP_EMP_ID)!), pagina, Environment.LIMITE_DE_LINHAS, totalCount, busca)
                ...
                .then((result) => {
                    ...
                    if (result instanceof Error) {
                        ...
                        if (totalCount === 0) {
                            ...
                            GrupoService.getAll(parseFloat(localStorage.getItem(Environment.APP_EMP_ID)!), 1, Environment.LIMITE_DE_LINHAS, 1, busca)
    ...
    const handleDelete = (id: number) => {
        if (window.confirm("Deseja excluir o registro ?")) {
            ...
            GrupoService.maintenece(JSON.stringify(row), "D")
                .then(result => {
                    if (result instanceof Error) {
                        alert(result.message);
                    }
        ...
        <LayoutBasePage titulo="Grupos"
            barraFeramentas={
                <FerramentasListagem
                    ...
                    onClickBtAdd={() => navigate('/grupos/detalhe/0')}
                />
            }>
                            ...
                            <Card>
                                ...
                                <CardActions>
                                    <Box display='flex' flexDirection='row' justifyContent='end' flex={1}>
                                        ...
                                        <IconButton size="small" onClick={() => { navigate(`/grupos/detalhe/${row.id}`, { state: JSON.stringify(row) }); }}>
                                            <Icon>edit</Icon>
                                        </IconButton>
                                    </Box>
                                </CardActions>
